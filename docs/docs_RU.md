...
Есть 1 тип скриптов-оберток:
1. Данные из шаблона (data in template)
...
14. `PATCH_ONLY_ALL_BIN_PATTERNS_EXIST`
    - сначала проверить, что все hex-паттерны есть в указанных файлах и только потом выполнить их замену
    - если хотя бы 1 hex-паттерн не найден - ни один файл не будет модифицирован
    - при этом поиск hex-паттернов фактически производится 2 раза и это может быть критично, если файлы тяжелые и находятся на медленных HDD
15. `PATCH_ONLY_ALL_BIN_PATTERNS_EXIST_1_TIME`
    - сначала проверить, что все hex-паттерны в указанных файлах встречаются только 1 раз и только потом выполнить их замену
    - если хотя бы 1 hex-паттерн не найден или найден более 1 раза - ни один файл не будет модифицирован
    - при этом поиск hex-паттернов фактически производится 2 раза и это может быть критично, если файлы тяжелые и находятся на медленных HDD
...
Пути к файлам могут содержать переменные окружения Windows, например `%temp%` или `%programfiles%` или `%appdata%` и т.д. Эти переменные будут корректно обработаны.

Если какого-либо файла из секции нет на диске, то обработка этого файла и паттернов для него пропускается без каких либо ошибок и секция анализируется дальше. Но если в шаблоне есть флаг `EXIT_IF_ANY_PATCH_BIN_FILE_NOT_EXIST` или `PATCH_ONLY_ALL_BIN_PATTERNS_EXIST` или `PATCH_ONLY_ALL_BIN_PATTERNS_EXIST_1_TIME`, то если хотя бы 1 файла нет на диске, то обработка всего шаблона завершится.
...
Здесь указываются строки или URL-адреса которые должны быть удалены из файла `hosts`. Если строка секции начинается с символа комментария `#` или с `127.0.0.1` или с `0.0.0.0` или с `::1`, то будет производится поиск точно такой же строки, без учета длины пробелов. В противном случае предполагается что указан URL-адрес (например adobe.io), и строки содержащие такой адрес (такое слово, то есть текст и границами текста будут пробелы или переносы строк) будут удалены.
...
Здесь указываются строки которые необходимо добавить в файл `hosts`. Обычно добавляются строки, чтобы заблокировать доступ к каким-то URL-адресам, поэтому в этой секции можно просто написать URL-адреса (разумеется каждый на новой строке) и в `hosts` будет добавлена строка `0.0.0.0 <URL-адрес>` (для блокировки доступа к URL по IPv4) и еще строка `::1 <URL-адрес>` (для блокировки доступа к URL по IPv6).

Если строка в секции начинается с `#` или с `127.0.0.1` или с `0.0.0.0` или с `::1`, то она будет добавлена в hosts без изменений. В противном случае строка считается непосредственным URL-адресом (например adobe.com) и в ее начало будет добавлено `0.0.0.0 `, что приведет к блокировке URL-адреса по IPv4, а следом будет добавлена такая же строка, но начинающаяся с `::1 `, что приведет к блокировке URL-адреса по IPv6.
...
Если в секции для добавления первая строка содержит текст `SEE_HERE_FIRST`, а потом ссылку на список и ссылка действительно есть и она доступна для скачивания, то скачается содержимое файла по указанной ссылке и будет обработано согласно тем же правилам, при этом содержимое самой секции использоваться не будет. Если же скачать файл не удастся, тогда будет обрабатываться содержимое секции. Например, первая строка может быть такой `SEE_HERE_FIRST https://a.dove.isdumb.one/list.txt`.

9. `files_or_folders_delete`

Здесь указываются абсолютные пути к файлам и папкам которые необходимо удалить.

Функция по работе с этой секцией действует грамотно - если у файла есть атрибут "Только чтение" то он будет снят перед удалением, чтобы не было ошибок. Если для удаления каких-то файлов или папок нужны будут права администратора, а у текущего процесса этих прав нет, то в блоке с логирующей информацией данной секции будет выведен сообщение об ошибке удаления указанных файлов и сообщение о том, что для их удаления нужны права администратора.
...
Здесь указываются пути к файлам (очевидно это должны быть `.exe`-файлы), правила для которых необходимо удалить из Windows Firewall. Предполагается что это будет выполняться для разблокировки доступа в интернет для программ для которых ранее доступ в есть был заблокирован. Будут удалены все правила фаервола для указанных путей.
...
Файлы [Block With Firewall](../utilities/BlockWithFirewall.cmd) в этой папке содержит функции ранее существовавшего скрипта-обертки типа "data inside" в виде файла `Start.cmd`. Думаю название файла явно говорят что делает этот скрипт. Текст/строки/данные с которыми он работает - находятся внутри него.
