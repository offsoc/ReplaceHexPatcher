...
В Windows предустановлены 4 "инструмента программирования" - CMD, Visual Basic Script, Powershell, C#.
...
8.  `CHECK_IF_ALREADY_PATCHED_ONLY`
   - только проверить наличие всех паттернов замены для всех файлов из секций `patch_bin` и `patch_text` без выполнения замены. То есть в указанных файлах будет производиться поиск паттернов замены, а не паттернов поиска. При этом паттерны замены будут "скомбинированы"/дополнены до длины паттернов поиска, если они меньше длины, а также wildcard-символы в паттернах заменены реальными символами из паттернов поиска. Другие секции (кроме секции с переменными для шаблона) не будут использованы
9. `EXIT_IF_NO_ADMINS_RIGHTS`
   - в самом начале работы патчера проверить наличие прав администратора у текущего выполняемого скрипта-патчера/процесса и если прав нет - завершить выполнение
...
11. `SHOW_SPACES_IN_LOGGED_PATTERNS`
   - при выводе информации о hex-паттернах - разделить пробелом каждый байтом (то есть каждые 2-мя hex-символа), чтобы паттерн отображался например так: 00 A4 32 02 00 00 00 00 E0 5E B4 00 00 10 00 00
...
2. В секциях `patch_text` и `patch_bin` нельзя указывать пути к несуществующим файлам.
   - Это связано с механизмом разделения строк на принадлежности к пути или паттерну поиска/замены. Перебираются все строки поочередно и если текст строки это действительный путь к файлу - то он используется как путь, а все остальные строки - как паттерны поиска/замены, пока не будет найден следующий действительный путь. Соответственно, если будет указан путь на несуществующий файл, то строка с таким путем будет рассматриваться как паттерн и вся дальнейшая обработка будет нарушена
   - Для обработки этой ситуации (проверка существования на диске всех файлов, которые необходимо пропатчить) рекомендую предварительно проверить наличие всех путей с помощью использования функции `Test-AllFilePaths` в секции `pre_powershell_code`
