...
- [x] Реализовать чтение шаблона из base64-кода
  - Как в случае, если это файл с base64-кодом, там и строка-аргумент
- [x] Добавить секции с `pre-cmd` и `pre-powershell` кодом и с `post-cmd` и `post-powershell` кодом
...
- [x] Возможно стоит добавить логику запуска `.ps1` файлов от имени администратора в скрипте-парсере
