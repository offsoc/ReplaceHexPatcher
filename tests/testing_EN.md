...
- [Testing functionality](#testing-functionality)
  ...
  - [How testing is performed](#how-testing-is-performed)
    - [Testing the "core" - byte search and replacement algorithms](#testing-the-core---byte-search-and-replacement-algorithms)
    - [The `search strategies` directory](#the-search-strategies-directory)


## Introductory information

The utility consists of several parts - the base/core and the "wrappers". It is necessary to test both. There are also 2 types of "wrappers" - CMD code and Powershell code. There probably aren't any frameworks or testing tools for the CMD code, so there's probably only output the function operation log via echo to the terminal window (which is what I did when testing my code).

For Powershell, there is at least 1 framework for writing tests - Pester (https://pester.dev / | https://github.com/pester/Pester)

Starting with version 2, the "core" of byte search and replacement is written in C# and there are many testing tools and frameworks for C#, but I am not strong in C# and do not want to devote time to its in-depth study. 

But in this project, so far, no frameworks or homemade tests are used. I am manually testing various code execution options (using the tool).

The Powershell version of the "wrapper", which is a template parser with instructions from this template, turned out to be too cumbersome, because I wrote it after writing the CMD version, in which it is impossible to split the functionality of the code into different files.
...
For example, you can add both direct paths and paths with an asterisk at the end to the firewall's file blocking section. Both existing and non-existing paths and compare which rules were in the firewall before executing the template and after. It's the same with other sections.

But with the byte search and replace algorithm, the situation is a bit different.

### Testing the "core" - byte search and replacement algorithms
...
Naturally, we need an option in which byte replacement will occur as quickly and error-free as possible. (that is, only the bytes that we specified will be replaced).

Since ChatGPT and other GPT exist, you can ask him to generate various algorithms for searching and replacing bytes on Powershell, and many of the functions generated by him will even work and work plausibly at first glance.
...
### The `search strategies` directory

For such tests of algorithms (strategies) of byte search + replacement, I have created a separate folder [search strategies](../tests/search%20strategies/), which will provide examples of working algorithms. ONLY WORKING ALGORITHMS/STRATEGIES. The difference between them should be only in the speed of execution. There will be files with a structure like [ReplaceHexBytesAll.ps1](../core/v1/ReplaceHexBytesAll.ps1), but with the "noise" cut out, that is, without the logic of checking administrator rights, removing "Read-only" attributes, and creating backups. In files with strategies/algorithms, there should only be a converter of hex patterns passed as arguments and their application to the file and the accompanying code without additional access checks, etc., so as not to clog the code and work only on the byte search algorithm/strategy.

Since v2, the core has been rewritten in C# and all search algorithms/strategies have been rewritten in C#. Since the goal of the project is primarily native byte search and replacement, the C# code must be executed by a compiler embedded in the system (in this case, Windows 10 or later). The file structure of their project [HexAndReplace] is used as a basis(https://github.com/jjxtra/HexAndReplace /) and the files BinaryReplacer.cs and HexAndReplaceApp.cs were left, which were later renamed HexHandler.cs and Launcher.cs. The 'build.cmd` file performs a simple compilation of both .cs files into an .exe file using csc.exe , embedded in the system. This file is needed solely to check for compilation errors.
